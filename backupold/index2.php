...
<div id="front_page_title">FRIDAY'S FRONT PAGE AND OTHER NEWS</div>-->
				...
				<!--VACATION MESSAGE--> <?php if($summerBreak) { ?>
				  <div style="border-width: 1px 0 1px 0; border-color: #CCCCCC; border-style: solid; padding: 15px 5px 5px 5px">
					...
					<p><font class="notetext">The Orient has concluded publication for the <?php echo $summerBreak_year; ?> academic year. Be sure to check out our <a href="http://orient.bowdoin.edu/orient/article2.php?date=2007-05-04&section=3&id=1">Year in Review</a>, and have a great summer!</font></p>
				</div>
				<br>
				<?php } ?>
			...
			<?php
				...
				$sqlQuery = "
					...
					where
						...
						article_priority = '$priority' and
						author.id = photo.photographer
					";
				...
				for($counter = 0; $counter < $limit; $counter=$counter+1)  
				{
					if ($row = mysql_fetch_array($result))  
					{
						...
						<font class="homeheadline"><?php if(($section != 1) && (strcmp($thumbFilename, "")!=0)) { ?><a href='images/<?php echo "$date/$largeFilename";?>' rel="lightbox" title="<?php echo $caption . "<br />" . $name;?>"><img border="1" class="thumb" src="images/<?php echo "$date/$thumbFilename" ?>" align="right" alt="Picture"></a><?php } ?>
						
<a class="homeheadline" href="article2.php?<?php echo "date=$date&section=$articleSection&id=$articlePriority" ?>">						
						<?php 
						$thumbFilename = "";
...
<font class="photorelated"> 
                        <a class="photorelated" href="article2.php?<?php echo "section=$photoSection&date=$date&id=$photoPriority" ?>">See feature article...</a></font><?php
}
?><br>
...
?>				  
                    ...
                    <tr> 
                      ...
                      <td width="97%"><font class="more"><a class="more" href="article2.php?date=<?php echo "$date&section=$articleSection&id=$articlePriority" ?>"><strong><em><?php echo $articleSeries ?></em></strong><strong><?php echo $articleType ?></strong><?php echo $articleTitle ?></a></font></td>
                    </tr>
<?php
...
<!-- headlines from nyt -->
                    <?php
                    	for ($k = 0; $k < mysql_num_rows($latestArticles); $k++) {
                    		...
                    		echo "<td width='97%'><font class='more'><a class='more' href='article2.php?date=$latestDate&section=" . mysql_result($latestArticles, $k, "section_id") . "&id=" . mysql_result($latestArticles, $k, "priority") . "'>" . mysql_result($latestArticles, $k, "title") . "</a></font></td></tr>";
                    	}
...
<!--                    <tr> 
                      ...
                      <td width="97%"><font class="more"><select onChange="document.location=this.value;">
                      	...
                      	<?php
/*                      		for ($k = 0; $k < mysql_num_rows($latestArticles); $k++) {
                      			echo "<option value='article2.php?date=$latestDate&section=" . mysql_result($latestArticles, $k, "section_id") . "&id=" . mysql_result($latestArticles, $k, "priority") . "'>" . (strlen(mysql_result($latestArticles, $k, "title")) < 40 ? mysql_result($latestArticles, $k, "title") : substr(mysql_result($latestArticles, $k, "title"), 0, 40) . "...") . "</option>"; 
                      		} */
                      	?>
                      </select></font></td> -->
