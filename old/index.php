<?php 
	...
	$fallBreak = false;
	$fallBreak_resumeDate = "Friday, October 17";
	
	$winterBreak = false;
	$winterBreak_resumeDate = "January 25, 2008";
	
	$springBreak = false;
	$springBreak_resumeDate = "March 28, 2008";
	
	$summerBreak = (mysql_result(mysql_query("SELECT int_value FROM settings WHERE name='summerBreak';"), 0, "int_value") == 1);
	$summerBreak_year = mysql_result(mysql_query("SELECT string_value FROM settings WHERE name='summerBreak_year';"), 0, "string_value");
	$summerBreak_year = "2007-2008";
	
	$fallBreak = (mysql_result(mysql_query("SELECT int_value FROM settings WHERE name='fallBreak';"), 0, "int_value") == 1);
	$fallBreak_resumeDate = mysql_result(mysql_query("SELECT string_value FROM settings WHERE name='fallBreak_resumeDate';"), 0, "string_value");
	
	$winterBreak = (mysql_result(mysql_query("SELECT int_value FROM settings WHERE name='winterBreak';"), 0, "int_value") == 1);
	$winterBreak_resumeDate = mysql_result(mysql_query("SELECT string_value FROM settings WHERE name='winterBreak_resumeDate';"), 0, "string_value");
	
	$winterBreak = false;
	$winterBreak_resumeDate = "January 23";
	
	$springBreak = (mysql_result(mysql_query("SELECT int_value FROM settings WHERE name='springBreak';"), 0, "int_value") == 1);
	$springBreak_resumeDate = mysql_result(mysql_query("SELECT string_value FROM settings WHERE name='springBreak_resumeDate';"), 0, "string_value");
	
	$fallBreak = false;
	$fallMessage = "Due to Thanksgiving break, there is no Orient this week.  Publication will resume December 5.<br />Congratulations to the Bowdoin field hockey team on a <a href='http://orient.bowdoin.edu/orient/article.php?date=2008-11-21&section=1&id=1'>repeat championship</a>!";
	...
	$dateQuery = "SELECT issue_date FROM issue WHERE ready='y' ORDER BY issue_date DESC LIMIT 0, 1";
	$latestDate = mysql_result(mysql_query($dateQuery), 0);
	
	$numMostViewed = mysql_result(mysql_query("SELECT * FROM settings WHERE name='mostViewed'"), 0, "int_value");
	
	$latestArticles = mysql_query("SELECT section_id, priority, author1, author2, author3, title, subhead, pullquote, views FROM article WHERE date='$latestDate' ORDER BY views DESC LIMIT 0, $numMostViewed");
...
<div id="front_page_title">FRIDAY'S FRONT PAGE AND OTHER NEWS</div>-->
				...
				<!--VACATION MESSAGE--> <?php if($summerBreak) { ?>
				  <div style="border-width: 1px 0 1px 0; border-color: #CCCCCC; border-style: solid; padding: 15px 5px 5px 5px">
					...
					<p><font class="notetext">The Orient has concluded publication for the <?php echo $summerBreak_year; ?> academic year. Be sure to check out our <a href="http://orient.bowdoin.edu/orient/article.php?date=2008-05-02&section=3&id=1">Year in Review</a>, and have a great summer!</font></p>
				</div>
				<br>
				<?php } ?>
				...
				<?php if ($fallBreak) { ?>
					...
					<font class="notetitle">THANKSGIVING BREAK</font>
					<p><font class="notetext"><?php echo $fallMessage; ?></font></p>
					</div>
					<br />
				<?php } ?>
				...
				<?php if ($winterBreak) { ?>
					<div style='border-width: 1px 0 1px 0; border-color: #ccc; border-style: solid; padding: 15px 5px 5px 5px'>
					<font class="notetitle">WINTER BREAK</font>
					<p><font class="notetext">The Bowdoin Orient will resume publication on <?php echo $winterBreak_resumeDate; ?>.  Have a great holiday!</font></p>
					</div>
					<br />
				<?php } ?>
				...
				<?php if (false) { ?>
					<div style='border-width: 1px 0 1px 0; border-color: #ccc; border-style: solid; padding: 15px 5px 5px 5px'>
					<font class="notetitle">SPRING BREAK</font>
					<p><font class="notetext">The Bowdoin Orient will resume publication on <?php echo $springBreak_resumeDate; ?>.</font></p>
					</div>
					<br />
				<?php } ?>
...
<?php
	# section info.
	$limit = mysql_result(mysql_query("SELECT int_value FROM settings WHERE name='numArticles'"), 0, "int_value");
...
You are currently using an old web browser that will likely display some formatting anomolies. This website renders beautifully in newer web browsers. Please <a href="/orient/browserupgrade.php">upgrade your web browser</a> to enjoy the <i>Orient's</i> website at its best.<br><br>
	...
	?>
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr> 
                      <td><a href="../images/<?php echo "$date/$photoLargeFilename"; ?>" style='text-decoration:none;' title="<?php echo $photoCaption . "<br />" . $photoCredit;?>" rel='lightbox'><img width='350' src="../images/<?php echo $date ?>/<?php echo $photoFilename ?>" style="border:1px solid black;" alt="Picture"></a></td>
                    </tr>
                    <tr>
                      <td height="3"></td>
                    </tr>
                    <tr> 
                      <td><div align="right"><font class="photoenlarge"><a class="photoenlarge" href='javascript: rs("ss","picturewindow.php?<?php echo "section=$photoSection&featuresection=$photoFeatureSection&date=$date&id=$photoPriority" ?>", 600,600);'><?php if($multiplePhotos==true) { ?>see more pictures / <?php } ?>click to enlarge</a></font></div></td>
                    </tr>
                    <tr>
                      <td height="3"></td>
                    </tr>
...
<p><table width="100%" border="0" cellpadding="2" cellspacing="0">
                    <tr> 
                      <td colspan="2" valign="top"><font class="morehead">This Week's Most-Viewed Articles</font></td>
                    </tr>
<!-- headlines from nyt -->
                    <?php
                    	for ($k = 0; $k < mysql_num_rows($latestArticles); $k++) {
                    		echo '<tr><td width="3%" valign="top"><font class="moredot">&#8226;</font></td>';
                    		echo "<td width='97%'><font class='more'><a class='more' href='article.php?date=$latestDate&section=" . mysql_result($latestArticles, $k, "section_id") . "&id=" . mysql_result($latestArticles, $k, "priority") . "'>" . mysql_result($latestArticles, $k, "title") . "</a></font></td></tr>";
                    	}
...
<!--                    <tr> 
                      ...
                      <td width="97%"><font class="more"><select onChange="document.location=this.value;">
                      	<option value="#">Most Popular Articles</option>
                      	<?php
/*                      		for ($k = 0; $k < mysql_num_rows($latestArticles); $k++) {
