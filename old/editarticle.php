<?php
	include("start.php");
	startcode("The Bowdoin Orient - Edit Article", false, false, $articleDate, $issueNumber, $volumeNumber);
...
?>
	...
	<table>
	<tr><td><label>URL of article: <input type="text" name="url" id="url" /></label></td></tr>
	<tr><td><label>Password: <input type="password" name="pwd" id="pwd" /></label></td></tr>
	<tr><td><input name="submit" type="submit" value="Edit Article" /></td></tr>
	</table>
	</form>
	
<?php } else if ($needToEdit) { ?>

	<p style="color: red;font-weight: bold;">Warning: All edits are final.  There is no "undo" once you commit.</p>
	<form action="editarticle.php" method="post" />
...
<?php } else if ($editedArticle) { ?>
	<p>You successfully edited <a href="article.php?date=<?php echo $date;?>&section=<?php echo $section;?>&id=<?php echo $id;?>"><?php echo $title;?></a></p>

<?php } else if ($deletedArticle) { ?>
	<p>You deleted the article. <a href='editarticle.php'>Edit another</a>?</p>
<?php } ?>
