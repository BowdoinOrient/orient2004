...
function getSectionFeaturePhoto($articleSection, $date) {
    $query = 
    "SELECT
		p.article_section,
		p.feature_section,
...
function commentForm() { ?>
	...
	<div class='comment-form'>
		<table>
			<tr>
				<td><label>Bowdoin Username: <div><input type='text' name='username' id='username' /></div></label></td>
			</tr>
			<tr>
		...
		<div>You must have a Bowdoin login in order to comment.  If you do not, there are a number of other ways to <a href='contact.php'>contact us</a>.
		<br />
		Questions or suggestions concerning comments? E-mail <a href='mailto:orient@bowdoin.edu'>orient@bowdoin.edu</a> with the subject line "Comments."</div>
		<br />
	</div>
<?php }
...
function spacer() {
	echo "<div class='spacer' style='height:10px'></div>";
}
...
function echoMedia($date, $section, $id) {
	...
	// $embeddables = embeddableQuery($date, $section, $id);
	...
	/* if (mysql_num_rows($embeddables) > 0) {
	   $anyembeds = true;
	} */
	...
	if ($row = mysql_fetch_array($photos)) {
		...
		// $photoCaption .= "<br><br>We are aware that photos are not displaying properly.  We are working on this, and will correct it as soon as possible.";
...
<?php }
                        if (false) { 
						// this is where the embed code will go once we iron out the quirks?>
                        <div class='otherembeds'>
                            <?php while ($row = mysql_fetch_array($embeddables)) {
                                echo $row['html'];
                                echo "<div class='embed-caption'>" . $row['caption'] . "</div>";
                            } ?>
                        </div>                 
...
function echoMobilePhoto($date, $section, $id) {
	$photos = photoQuery($date, $section, $id);
	if ($row = mysql_fetch_array($photos)) {
		$anyphotos = true;
		$articleImage = $row['article_filename'];
	...
	if ($anyphotos) { ?>
		<br />
		<img src='images/<?php echo "$date/$articleImage"; ?>'>
		<div class='photocredit last'>
			<?= strtoupper($photoCredit); ?>
		</div>
	<?php }
}
...
function authorsLinksMobile($a1, $a2, $a3) {
	// Returns the authors string. No links, no jobs.
	$authors = "";
	if ($a1) {
		$authors = "By ";
		$authors .= $a1;
	...
	return "<p class='bottom'>" . $authors . "</p>";
}
...
function echoArticleOptions($date, $section, $id, $title) { ?>
	<div class='articleoptions'>
		<ul class='morestories'>
			...
			<li><a href='letters.php'>Send a letter to the editor</a></li>
		</ul>
	</div>
<?php }
...
function authors($a1, $a2, $a3, $job) {
	...
	// For Zoë :)
	$authors = str_replace("ë", "Ë", $authors); 
...
function getSectionArticles($section, $date, $noneditorials) {
	...
	// Until the previous one is fixed, only one author.
	...
	SELECT
		section_id,
		priority,
		author.name AS author1,
		job.name AS jobname,
		article.title,
		article.pullquote,
		articletype.name AS type
	FROM
		article, articletype, author, job
	WHERE article.date = '$date'
	AND article.author1 = author.id
	AND article.author_job = job.id
	AND article.type = articletype.id ";
	if ($noneditorials) {
		$query .= "AND article.type != '2' ";
	}
	$query .= 
...
function getFeaturePhoto($date) {
	...
	if($row = mysql_fetch_array($result)) 
	{
		...
		if ($relatedArticle) {
			$photoCaption .= " <a href='article.php?date=$date&amp;section=$photoSection&amp;id=$photoPriority'>See feature article...</a>";
		}
...
function articleEmailLink($date, $section, $priority) {
	return "article.php?date=$date&section=$section&id=$priority";
}
...
function embeddableQuery($date, $section, $id) {
    $query = 
    "SELECT
    ...
    FROM
        embedded em
    WHERE
        em.article_date = '$date' AND
        em.article_section = '$section' AND
        em.article_priority = '$id'
    ";
    return mysql_query($query);
}
...
function echoSectionArticle($row) {
	...
	echo "<div class='sectionarticle $photoClass'>";
