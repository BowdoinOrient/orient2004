...
$ldapConnection = ldap_connect("bowdoincollege.edu", 389);
if($ldapConnection) {
	...
	$ldapBind = ldap_bind($ldapConnection, "$username@bowdoincollege.edu", $password);
	error_reporting($errorLevel);
	if($ldapBind) {
		// $sr = ldap_search($ldapConnection, "ou=People, o=Bowdoin College, c=US", "uid=".$username);  
		$sr = ldap_search($ldapConnection, "OU=People,DC=bowdoincollege,DC=edu", "sAMAccountName=".$username);
		$info = ldap_get_entries($ldapConnection, $sr);
		$realname = mysql_real_escape_string($info[0]['cn'][0]);
		if($info["count"]!=1) {
		...
		// Add comment
		addComment($username, $username . "@bowdoin.edu", $comment, $date, $section, $id, $realname);
		exit;
	...
	{
		// $sr = ldap_search($ldapConnection, "ou=People, o=Bowdoin College, c=US", "uid=".$username);  
		$sr = ldap_search($ldapConnection, "OU=People,DC=bowdoincollege,DC=edu", "sAMAccountName=".$user);
		$info = ldap_get_entries($ldapConnection, $sr);
		if($info["count"]!=1)
		{
			echo "Your username, password, or potentially both were incorrect.  Please try again with a valid Bowdoin ID.";
...
function addComment($username, $email, $comment, $date, $section, $id, $realname) {
	$commentSecret = rand(0, 1000);
	$commentQuery = 
		"INSERT INTO
			comment
			(`article_date`, `article_section`, `article_priority`, `username`, `email`, `comment`, `secret`, `comment_date`, `realname`)
		VALUES
			('$date', '$section', '$id', '$username', '$email', '$comment', '$commentSecret', NOW(), '$realname')
		";
	if ($results = mysql_query($commentQuery)) {
		...
		"\nYou can read it in context here: http://orient.bowdoin.edu/orient/" . articleEmailLink($date, $section, $id) . "#comments .\n";
		$subject = "Orient Comment Report";
		$from = "From: OrientBot <orient@bowdoin.edu>";
		$to = "seth.glickman@bowdoin.edu, seth.glickman@gmail.com, cybertaur1@gmail.com, orient@bowdoin.edu, ctucker@bowdoin.edu, swalder@bowdoin.edu, pgrosswe@bowdoin.edu";
